...
-- Flat or room ko option halna ko lagi add gareko 
-- Alter listings table to add the type column (room or flat)
alter table public.listings
add column type text not null default 'room';  -- Default to 'room', you can change as per need
...
- other 
drop policy if exists "images_upload_authenticated" on storage.objects;

create policy "images_upload_authenticated"
on storage.objects
for insert
to authenticated
with check (
